...
def query_span_f1(start_preds, end_preds, match_logits, start_label_mask, end_label_mask, match_labels, flat=False):
    ...
    tp = (match_labels & match_preds).long().sum().detach().cpu().numpy()
    fp = (~match_labels & match_preds).long().sum().detach().cpu().numpy()
    fn = (match_labels & ~match_preds).long().sum().detach().cpu().numpy()
    if tp == 0.:
        return 0.
    pre = tp / (tp + fp)
    recall = tp / (tp + fn)
...
def logits2answer(start_logits, end_logits, span_logits):
    ...